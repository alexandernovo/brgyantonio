<script>
    ...
    let certificationBrgyData = [];

    certificationBrgyOptions = {
        processing: true,
        ...
        columns: [{
            ...
            },
            {
                ...
                className: 'text-nowrap p-2 text-center align-middle',
                data: 'place_of_birth' // Matches migration
            },
            {
                title: 'SEX',
                className: 'text-nowrap p-2 text-center align-middle',
            ...
            },
            {
                ...
                <div class="d-flex gap-1 justify-content-center">
                    <a href="{{ route('viewBrgyCertification') }}?certification_id=${row.certification_id}" class="btn btn-dark btn-sm printButton px-2" style="background-color: #1A212B !important"><i style="font-size: 15px" class="bi bi-printer-fill"></i></a>
        ...
        initComplete: function(settings, json) {
            ...
            $("#certificationTableBrgy_wrapper .dt-length")
    ...
    function renderCertificationTableBrgy() {
        if (certificationTableBrgy) {
            certificationTableBrgy.destroy();
        }

        certificationTableBrgy = new DataTable('#certificationTableBrgy', certificationBrgyOptions)
    }
    ...
    $(document).on("click", "#addCertificationBrgy", function() {
    ...
    $(document).on('click', 'table.dataTable tbody tr', function() {
        ...
        const rowData = certificationTableBrgy.row(this).data();
    ...
    function reloadBrgyCertification() {
        if (certificationTableBrgy) {
            certificationTableBrgy.ajax.reload(null, false);
        } else {
            renderCertificationTableBrgy();
        }
    }
    ...
    $(document).on("click", ".editButton", function(e) {
        ...
        let find_data = certificationBrgyData.find(x => x.certification_id == certification_id);
        if (find_data) {
            $("#certificationForm")[0].reset();
    ...
    certificationBrgyOptions.drawCallback = function() {
