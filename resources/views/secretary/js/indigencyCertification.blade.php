<script>
    ...
    let dateFromIndigency = '';
    let dateToIndigency = '';
    let selectedIndigency = '';
    let certificationTableIndigency = null;
    let selectedCertificationRow = null;
    let selectedCertificationId = null;
    let certificationIndigencyData = [];

    certificationIndigencyOptions = {
        processing: true,
        ...
        ajax: {
            ...
            data: function(d) {
                d._token = '{{ csrf_token() }}';
                d.dateFrom = dateFromIndigency;
                d.dateTo = dateToIndigency;
                d.type = "indigency";
                d.letter = selectedIndigency;
            },
            dataSrc: function(json) {
                certificationIndigencyData = json.data;
                return json.data;
            }
        },
        columns: [{
            ...
            },
            {
                title: 'FAMILY INCOME',
                className: 'text-nowrap p-2 text-center align-middle',
                data: 'familyincome' // Matches migration
            },
            {
                title: 'DATE ISSUED',
                className: 'text-nowrap p-2 text-center align-middle',
            ...
            },
            {
                ...
                <div class="d-flex gap-1 justify-content-center">
                    <a href="{{ route('viewIndigencyCertification') }}?certification_id=${row.certification_id}" class="btn btn-dark btn-sm printButton px-2" style="background-color: #1A212B !important"><i style="font-size: 15px" class="bi bi-printer-fill"></i></a>
        ...
        initComplete: function(settings, json) {
            ...
            <div class="d-flex align-items-center gap-2 flex-wrap">
                <div class="input-group date-filter-box" style="width:auto;">
                    ...
                    <input type="date" value="{{ date('Y-m-d') }}" class="form-control" id="certDateFromIndigency">
                    <span class="input-group-text">To</span>
                    <input type="date" value="{{ date('Y-m-d') }}" class="form-control" id="certDateToIndigency">
            ...
            $("#certificationTableIndigency_wrapper .dt-length")
    ...
    function renderCertificationTableIndigency() {
        if (certificationTableIndigency) {
            certificationTableIndigency.destroy();
        }

        certificationTableIndigency = new DataTable('#certificationTableIndigency', certificationIndigencyOptions)
    }

    $(document).ready(function() {
        renderCertificationTableIndigency();
    })

    $(document).on("click", "#addCertificationIndigency", function() {
    ...
    $(document).on('click', 'table.dataTable tbody tr', function() {
        ...
        const rowData = certificationTableIndigency.row(this).data();
    ...
    $(document).on("click", ".editButton", function(e) {
        ...
        let find_data = certificationIndigencyData.find(x => x.certification_id == certification_id);
        if (find_data) {
            $("#certificationForm")[0].reset();
    ...
    certificationIndigencyOptions.drawCallback = function() {
    ...
    $(document).on("click", ".btn-reload-table", function() {
        dateFromIndigency = '';
        dateToIndigency = '';
        selectedLetterIndigency = '';

        certificationIndigencyOptions.ajax.data.dateFrom = dateFromIndigency;
        certificationIndigencyOptions.ajax.data.dateTo = dateToIndigency;
        certificationIndigencyOptions.ajax.data.selectedLetterIndigency = selectedLetterIndigency;
        $(".alpha-btn").removeClass("active");
        certificationTableIndigency.column(1).search('').draw();
        reloadIndigencyCertification();
    })

    $(document).on("click", "#btnCertFilter", function() {
        dateFromIndigency = $("#certDateFromIndigency").val();
        dateToIndigency = $("#certDateToIndigency").val();
        certificationIndigencyOptions.ajax.data.dateFrom = dateFromIndigency;
        certificationIndigencyOptions.ajax.data.dateTo = dateToIndigency;
        reloadIndigencyCertification();
    })
