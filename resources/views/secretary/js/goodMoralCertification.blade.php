<script>
        ...
        initComplete: function(settings, json) {
            ...
            <div class="d-flex align-items-center gap-2 flex-wrap">
                <div class="input-group date-filter-box" style="width:auto;">
                    ...
                    <input type="date" value="{{ date('Y-m-d') }}" class="form-control" id="certDateFromMoral">
                    <span class="input-group-text">To</span>
                    <input type="date" value="{{ date('Y-m-d') }}" class="form-control" id="certDateToMoral">
    ...
    function reloadMoralCertification() {
        if (certificationTableMoral) {
    ...
    $(document).on("click", ".btn-reload-table", function() {
        ...
        certificationMoralOptions.ajax.data.dateFrom = dateFromMoral;
        certificationMoralOptions.ajax.data.dateTo = dateToMoral;
        certificationMoralOptions.ajax.data.selectedLetterMoral = selectedLetterMoral;
        $(".alpha-btn").removeClass("active");
        certificationTableMoral.column(1).search('').draw();
        reloadMoralCertification();
    })

    $(document).on("click", "#btnCertFilter", function() {
        dateFromMoral = $("#certDateFromMoral").val();
        dateToMoral = $("#certDateToMoral").val();
        certificationMoralOptions.ajax.data.dateFrom = dateFromMoral;
        certificationMoralOptions.ajax.data.dateTo = dateToMoral;
        reloadMoralCertification();
    })
