<script>
    ...
    let dateFromLivestock = '';
    let dateToLivestock = '';
    let selectedLivestock = '';
    let certificationTableLivestock = null;
    ...
    certificationLivestockOptions = {
        ...
        ajax: {
            ...
            data: function(d) {
                d._token = '{{ csrf_token() }}';
                d.dateFrom = dateFromLivestock;
                d.dateTo = dateToLivestock;
                d.type = "livestock";
                d.letter = selectedLivestock;
            },
            dataSrc: function(json) {
                certificationLivestockData = json.data;
                return json.data;
            }
        },
        columns: [{
...

            {
                ...
                className: 'text-nowrap p-2 text-center align-middle',
                data: 'nolivestock' // Matches migration
            },
            {
                title: 'AGE CLASSES',
                className: 'text-nowrap p-2 text-center align-middle',
                data: 'ageclasses' // Matches migration
            },
            {
                title: 'COLOR',
                className: 'text-nowrap p-2 text-center align-middle',
            ...
            },
            {
                ...
                <div class="d-flex gap-1 justify-content-center">
                    <a href="{{ route('viewLiveStockCertification') }}?certification_id=${row.certification_id}" class="btn btn-dark btn-sm printButton px-2" style="background-color: #1A212B !important"><i style="font-size: 15px" class="bi bi-printer-fill"></i></a>
        ...
        initComplete: function(settings, json) {
            ...
            $("#certificationTableLivestock_wrapper .dt-length")
    ...
    function renderCertificationTableLivestock() {
    ...
    $(document).ready(function() {
        renderCertificationTableLivestock();
    })

    $(document).on("click", "#addCertificationLivestock", function() {
    ...
    $(document).on('click', '#editCertificationLivestock', function() {
        ...
        window.location =
            `{{ route('editLiveStockCertification') }}?certification_id=${selectedCertificationId}`
    });
    ...
    $(document).on('click', 'table.dataTable tbody tr', function() {
        ...
        const rowData = certificationTableLivestock.row(this).data();
    ...
    $(document).on("click", ".editButton", function(e) {
        ...
        let find_data = certificationLivestockData.find(x => x.certification_id == certification_id);
        if (find_data) {
            $("#certificationForm")[0].reset();
